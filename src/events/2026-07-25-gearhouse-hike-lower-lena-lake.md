---
layout: event.njk
title: "Group Hike: Lower Lena Lake, Olympics (via Gearhouse)"
date: 2026-06-22
start: 2026-07-25T08:00:00-07:00
end: 2026-07-25T19:00:00-07:00
venue: "Lena Lake Trailhead, Olympic National Forest"
address: "Lena Lake Trailhead, Hamma Hamma Road, Eldon, WA 98555"
url: "https://www.joingearhouse.com/"
source: "ics:6e6dd0ec-e4b2-4d4d-ac18-bbfc0414289c"
region: other
cost: "Included with Gearhouse membership — RSVP on the calendar"
tags: [hiking, outdoors, mountains, lake, social]
---

A change of mountain range for once: **Gearhouse** is leading a group hike to **Lower
Lena Lake** in the **Olympics** on Saturday, July 25. It's a classic introduction to the
range — about **6.5 miles round trip with 1,500 feet of gain**, rated intermediate,
around four hours of hiking through second-growth forest to a big, clear lake ringed by
peaks. A manageable distance for a serious-feeling destination, and a fine swimming or
lunch spot at the top.

The catch is the drive: the Olympics are across the water, so the trailhead off **Hamma
Hamma Road is about 2.5 hours** from the Eastside (south to Tacoma, then up the west side
of the Sound). That's a full day's commitment, which is exactly why doing it with the
club helps — a carpool makes the miles social instead of solitary, and you get a known
route into a range a lot of Eastsiders rarely make time for.

RSVP on the [Gearhouse calendar](https://www.joingearhouse.com/) for the meet time and
carpool plan. Pack a full day-hike kit: sturdy shoes, layers, water and food, and the
[Ten Essentials](https://www.nwac.us/). If the weather's warm, bring a swimsuit and a
towel — the lake is the reward.
